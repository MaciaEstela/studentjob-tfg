...
<div class="offers-container-wrapper">
	<div class="offers-container-header-wrapper container mb-7">
		...
		<div class="offer-search offers-search-container row mt-5">
			...
			<aui:input name="keywords" wrapperCssClass="col-12 col-md-3" type="text" label="studentjob.offers.keywords"/>
			
			<%-- Region field. --%>
			<aui:select name="region" wrapperCssClass="col-12 col-md-3"  label="studentjob.offers.region">
				<aui:option label="studentjob.offers.entire-spain" value=""/>
			...
			<%-- Degree field. --%>
			<aui:select name="degree" wrapperCssClass="col-12 col-md-3"  label="studentjob.offers.degree">
				<aui:option label="studentjob.offers.all-degrees" value=""/>
			...
			<%-- Preference field. --%>
			<aui:select name="preference" wrapperCssClass="col-12 col-md-3"  label="studentjob.offers.preference">
				<aui:option label="studentjob.offers.indifferent" value=""/>
				<c:forEach items="${preferences}" var="preference">
					<aui:option label="studentjob.offers.preference.${preference}" value="${preference}"/>
				</c:forEach>
			</aui:select>
...
<div class="container">
	...
	<div class="offers-cards" id="<portlet:namespace />offers-list">
		<c:forEach items="${offersDTO}" var="offer">
			<%@ include file="/offer/public/card.jspf" %>
		</c:forEach>
	</div>
		...
		function updateOffersAuxData(){
			...
			AUI().use('aui-io-request', function(A){
				A.io.request('${getOffersAuxDataURL}', {
					...
					on: {
						success: function() {
							console.log("rebo " + this.get('responseData'))
							if (this.get('responseData') && this.get('responseData') !== ""){
