...
<c:choose>
	<c:when test="<%= themeDisplay.isSignedIn() %>">
		...
		if (themeDisplay.isShowMyAccountIcon() && (themeDisplay.getURLMyAccount() != null)) {
			...
			isAdmin = themeDisplay.getPermissionChecker().isOmniadmin();
		}
		%>
	...
	<c:otherwise>
		...
		<div class="container-md studentjob-login">
			<div class="studentjob-login__wrapper">
				<div class="studentjob-login__login-container">
					...
					<portlet:actionURL name="/login/login" secure="<%= PropsValues.COMPANY_SECURITY_AUTH_REQUIRES_HTTPS || request.isSecure() %>" var="loginURL">
						<portlet:param name="vcRenderCommandName" value="/login/login" />
					</portlet:actionURL>
					...
					<aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="<%= formName %>" onSubmit="event.preventDefault();" validateOnBlur="<%= false %>">
						...
						<div class="studentjob-login__btn-wrapper mb-2">
							<aui:button-row>
				...
				<div class="register-container text-center">
					<h2 class="register-container__title"><liferay-ui:message key="studentjob.no.account.yet"/></h2>
					<div class="studentjob-login__register-wrapper">
						<div class="mt-3">
							<a href="/register" class="btn register-container__btn">
								<liferay-ui:message key="studentjob.create.account.now" />
							</a>
						</div>
						<img class="floating-large mt-6" src="/o/studentjob-theme/images/register.png">
					</div>
				</div>
			</div>
		</div>
